...
const string strMessageMagic = "BLAST Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    // Changed 100 to 300 for longer pszTimestamp string
    if (tx.IsCoinBase())
    {
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BLAST additional fees, %d delta bytes\n",
...
//
// BLAST check header auxpow proof of work
...
// BLAST: block header template's for auxpow integration
template<typename T>
...
{
    // BLAST check auxpow first then fallback to normal check
